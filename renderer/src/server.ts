// src/server.ts
// MJPEG renderer server with *open CORS* for all origins + S3 avatar loading
...
import { currentViseme, visemeOpenAmount } from "./visemes.js";
...
// ── CORS: accept ALL origins (no credentials)
app.use(
  cors({
    origin: "*",
...
// Explicit preflight (always allow *)
...
app.post("/render/start", async (req, res) => {
  try {
    ...
    if (BUCKET && avatarKey) {
      try {
        const buf = await s3GetBuffer(BUCKET, avatarKey);
    ...
    sessions.set(id, {
      id,
      avatarImg,
      visemes: Array.isArray(visemes) ? visemes : [],
      startTs: Date.now(),
